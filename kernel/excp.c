...
void handle_smode_exception(unsigned int cause, struct trap_frame * tfr) {
    ...
    // Lazy allocation: S-mode access (e.g., elf_load memcpy with SUM) into
    // an unmapped user-range page faults here.  Route to handle_umode_page_fault
    // before panicking.
...
void handle_umode_exception(unsigned int cause, struct trap_frame * tfr) {
    ...
    const char * name = (cause < sizeof(excp_names)/sizeof(excp_names[0]))
        ? excp_names[cause] : NULL;
    char msgbuf[96];
    snprintf(msgbuf, sizeof(msgbuf),
             "Unhandled U-mode exception '%s' (cause=%u) at %p stval=%p",
             name ? name : "?", cause, (void*)tfr->sepc,
             (void*)csrr_stval());
    panic(msgbuf);
}
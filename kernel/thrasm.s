...
_thread_startup:
        # First entry into a freshly-spawned thread.  thread_spawn stashed
        # up to 8 user args in s0..s7 and the entry function pointer in s8.
        # _thread_swtch restored those as part of the context switch, so
        # we just shuffle them into the C ABI argument registers and jump.
        # The previous thread typically swtch'd to us with SIE=0 (running_
        # thread_suspend disables before _thread_swtch), so re-enable
        # interrupts globally before entering user code -- otherwise the
        # new thread starves any device that needs an ISR.
        csrsi   sstatus, 2          # set sstatus.SIE
        mv      a0, s0
        mv      a1, s1
        mv      a2, s2
        mv      a3, s3
        ...
        mv      a6, s6
        mv      a7, s7
        jalr    s8                  # call entry(a0..a7)
        call    thread_exit         # if entry returns, fall into exit
        # thread_exit is noreturn -- this point is unreachable

...
smode_trap_entry_from_umode:
        ...
        # sp = kernel-side trap frame slot (was sscratch); sscratch = user sp.
        # Save all user GPRs into the trap frame.
        ...
        # Save user sp (currently in sscratch) and clear sscratch (S-mode).
        csrr    t0, sscratch
        sd      t0, SP(sp)
        csrw    sscratch, zero

        # Save sstatus, sepc, instret.
        csrr    t0, sstatus
        sd      t0, SSTATUS(sp)
        csrr    t0, sepc
        sd      t0, SEPC(sp)
        ...
        # Restore kernel gp/tp from the thread anchor at sp + TFRSZ.
        ld      gp, (TFRSZ + KGP)(sp)
        ld      tp, (TFRSZ + KTP)(sp)

        # Set fp so backtrace works.
        addi    fp, sp, TFRSZ

        # Dispatch.  scause >= 0 -> exception, < 0 -> interrupt.
        csrr    a0, scause
        mv      a1, sp
        bgez    a0, .Lumode_excp
        slli    a0, a0, 1
...
.Lumode_resume:
        # Return to U-mode via trap_frame_jump.
        mv      a0, sp
        mv      a1, sp
        j       trap_frame_jump
